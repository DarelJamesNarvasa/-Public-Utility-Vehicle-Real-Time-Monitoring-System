...
def handle_mqtt_message(client, userdata, message):
    ...
    # Stored Database
    data = Database(8.230236, 124.2633, 1, "02/12/2023", "10:43", "embard", "John")
...
def index():
    ...
    passengerlist_devOne = database_session.query(Database).filter_by(rakalId=1).filter(Database.rfid.isnot(None)).all()
    ...
    if deviceOne_coordinates:
        ...
        for marker in passengerlist_devOne:
    ...
    passengerlist_devTwo = database_session.query(Database).filter_by(rakalId=2).filter(Database.rfid.isnot(None)).all()
    ...
    if deviceTwo_coordinates:
        ...
        for marker in passengerlist_devTwo:
            coord = [marker.latitude, marker.longitude]
            folium.Marker(
                location=coord,
                tooltip=f"{marker.rfid}",
    ...
    passengerlist_devThree = database_session.query(Database).filter_by(rakalId=3).filter(Database.rfid.isnot(None)).all()
    ...
    if deviceThree_coordinates:
        ...
        for marker in passengerlist_devThree:
            coord = [marker.latitude, marker.longitude]
            folium.Marker(
                location=coord,
                tooltip=f"{marker.rfid}",
    ...
    return render_template('index.html', map1_html=map1_html, map2_html=map2_html, map3_html=map3_html,
                           passengerlist_devOne=passengerlist_devOne, passengerlist_devTwo=passengerlist_devTwo, passengerlist_devThree=passengerlist_devThree)
